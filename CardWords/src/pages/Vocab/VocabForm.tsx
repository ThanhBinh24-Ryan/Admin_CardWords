...
//       // Convert image to base64 để lưu trực tiếp
//       const reader = new FileReader();
//       reader.onloadend = () => {
//         const base64String = reader.result as string;
//         handleChange('img', base64String);
//       };
//       reader.readAsDataURL(file);
//     }
//   };
...
//       // Convert audio to base64 để lưu trực tiếp
//       const reader = new FileReader();
//       reader.onloadend = () => {
//         const base64String = reader.result as string;
//         handleChange('audio', base64String);
//       };
//       reader.readAsDataURL(file);
//     }
//   };
...
//                       <div className="space-y-3">
//                         <div className="relative inline-block">
//                           <img src={imagePreview} alt="Preview" className="h-40 mx-auto object-cover rounded-xl shadow-lg" />
//                         </div>
//                         <button
//                           type="button"
//                           onClick={removeImage}
...
const VocabForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        setUploading(true);
        const imageResponse = await vocabService.uploadImage(imageFile);
        imgUrl = Object.values(imageResponse.data)[0] as string;
      }

      if (audioFile) {
        const audioResponse = await vocabService.uploadAudio(audioFile);
        audioUrl = Object.values(audioResponse.data)[0] as string;
      }
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // For now, keep the file for upload later
      handleChange('img', ''); // Clear base64, will upload file
    }
  };
  ...
  const handleAudioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setAudioFile(file);
      handleChange('audio', ''); // Clear base64, will upload file
    }
  };
